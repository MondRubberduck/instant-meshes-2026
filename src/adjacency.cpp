            ...
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
                ...
                do {
                    ...
                    if (it == 0) {
                        ...
                        Float final_weight = std::clamp((Float) cot_weight * 0.5f, (Float) 1e-4f, (Float) 100.0f);
                        *ptr++ = Link(F(prev_idx, f), (float) final_weight);
                    }
                    uint32_t opp = E2E[edge], next = dedge_next_3(opp);
                    if (opp == INVALID || next != stop) {
                        ...
                        Float final_weight = std::clamp((Float) cot_weight * 0.5f, (Float) 1e-4f, (Float) 100.0f);
                        *ptr++ = Link(F(next_idx, f), (float) final_weight);
                        if (opp == INVALID)
                            break;
                    }
                    edge = next;
