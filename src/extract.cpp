...
void extract_faces(std::vector<std::vector<TaggedLink> > &adj, MatrixXf &O,
                ...
                [&](const tbb::blocked_range<uint32_t> &range) {
                    ...
                    for (uint32_t i = range.begin(); i != range.end(); ++i) {
                        ...
                        if (adj_new[i].size() > 0 && !is_crease) {
                            ...
                            N_prime.col(i) = n;
                        } else if (is_crease && adj_new[i].size() > 0) {
                            // Tangential feature curve relaxation: smooth edge lengths along crease without rounding off sharp lines
                            std::vector<uint32_t> crease_nbrs;
                            for (auto j : adj_new[i]) {
                                if (crease.find(j) != crease.end())
                                    crease_nbrs.push_back(j);
                            }
                            if (crease_nbrs.size() == 2) {
                                Vector3f c_avg = (O.col(crease_nbrs[0]) + O.col(crease_nbrs[1])) * 0.5f;
                                Vector3f t_dir = (O.col(crease_nbrs[1]) - O.col(crease_nbrs[0]));
                                Float t_norm = t_dir.norm();
                                if (t_norm > 1e-6f) {
                                    t_dir /= t_norm;
                                    Vector3f pos = O.col(i) + t_dir * (t_dir.dot(c_avg - O.col(i)) * 0.5f);
                                    if (bvh && bvh->F()->size() > 0) {
                                        Ray ray1(pos,  N.col(i), 0, scale / 2);
                                        Ray ray2(pos, -N.col(i), 0, scale / 2);
                                        uint32_t idx1 = 0, idx2 = 0;
                                        Float t1 = 0, t2 = 0;
                                        bvh->rayIntersect(ray1, idx1, t1);
                                        bvh->rayIntersect(ray2, idx2, t2);
                                        if (std::min(t1, t2) < scale * 0.5f)
                                            pos = t1 < t2 ? ray1(t1) : ray2(t2);
                                    }
                                    O_prime.col(i) = pos;
                                } else {
                                ...
                                // Junction or endpoint: lock rigidly
                                O_prime.col(i) = O.col(i);
                            }
    ...
    // QEx-style final topological verification and hardening
    ExtractionSanitizer::sanitize_final_mesh(F, O, N, Nf);
