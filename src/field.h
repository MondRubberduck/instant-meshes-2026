...
extern Float
optimize_orientations(MultiResolutionHierarchy &mRes, int level, bool extrinsic,
                      int rosy, const std::function<void(uint32_t)> &progress);

extern Float optimize_positions(MultiResolutionHierarchy &mRes, int level,
...
extern void
compute_orientation_singularities(const MultiResolutionHierarchy &mRes,
...
class Serializer;
class Optimizer : public SolverInterface {
public:
  Optimizer(MultiResolutionHierarchy &mRes, bool interactive);
  virtual ~Optimizer() = default;

  void save(Serializer &state) override;
  void load(const Serializer &state) override;
  ...
  bool active() override { return mOptimizePositions | mOptimizeOrientations; }
  inline void notify() override { mCond.notify_all(); }

  void optimizeOrientations(int level) override;

  void optimizePositions(int level) override;

  void wait() override;

  void setExtrinsic(bool extrinsic) override { mExtrinsic = extrinsic; }
  bool extrinsic() const override { return mExtrinsic; }

  void setRoSy(int rosy) override { mRoSy = rosy; }
  int rosy() const override { return mRoSy; }
  void setPoSy(int posy) override { mPoSy = posy; }
  int posy() const override { return mPoSy; }
  void setLevel(int level) override { mLevel = level; }
  int level() const override { return mLevel; }
  Float progress() const override { return mProgress; }

#ifdef VISUALIZE_ERROR
  const VectorXf &error() { return mError; }
#endif

  void moveSingularity(const std::vector<uint32_t> &path,
                       bool orientations) override {
