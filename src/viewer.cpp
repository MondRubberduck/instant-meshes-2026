...
Viewer::Viewer(bool fullscreen, bool deterministic)
    ...
    new Label(window, "Curvature adaptivity", "sans-bold");
    Widget *adaptivityPanel = new Widget(window);
    adaptivityPanel->setLayout(
        new BoxLayout(Orientation::Horizontal, Alignment::Middle, 0, 10));

    mAdaptivitySlider = new Slider(adaptivityPanel);
    mAdaptivitySlider->setValue(0.0f);
    mAdaptivitySlider->setId("adaptivitySlider");
    mAdaptivitySlider->setFixedWidth(60);

    mAdaptivityBox = new TextBox(adaptivityPanel);
    mAdaptivityBox->setFixedSize(Vector2i(80, 25));
    mAdaptivityBox->setValue("0.00");
    mAdaptivityBox->setId("adaptivityBox");

    mAdaptivitySlider->setCallback([&](Float value) {
        mAdaptivity = value;
        char tmp[16];
        snprintf(tmp, sizeof(tmp), "%.2f", value);
        mAdaptivityBox->setValue(tmp);
    });
    ...
    mFilterSingularitiesBox = new CheckBox(exportPopup, "Singularity Filter");
    mFilterSingularitiesBox->setTooltip("Cancel opposite-sign dipole pairs to eliminate helical spiral loops");
    mFilterSingularitiesBox->setChecked(true);
    mFilterSingularitiesBox->setId("filterSingularitiesBox");
...
void Viewer::setTargetVertexCount(uint32_t v) {
    ...
    uint32_t coarse_face_target = (posy == 4 && mPureQuadBox && mPureQuadBox->checked()) ? std::max(1, face_count / 4) : face_count;
    Float face_area = mMeshStats.mSurfaceArea / coarse_face_target;
...
void Viewer::extractMesh() {
    ...
    // Modern QuadriFlow-inspired dipole regularization
    if (mFilterSingularitiesBox && mFilterSingularitiesBox->checked() && rosy == 4 && posy == 4) {
        SingularityFilter::regularize_singularities(mRes, rosy, mRes.scale() * 2.5f);
    }
    ...
    // Production Quality Gates & Mesh Metrics
    MeshQualityReport report = MeshQualityReporter::analyze(mF_extracted, mV_extracted, mN_extracted, mNf_extracted);
    MeshQualityReporter::print_report(report);
...
void Viewer::loadInput(std::string filename, Float creaseAngle, Float scale,
    ...
    if (filename.empty()) {
        ...
        if (filename == "")
            return;
    } else if (extension != ".ply" && extension != ".obj" && extension != ".fbx" && extension != ".aln")
