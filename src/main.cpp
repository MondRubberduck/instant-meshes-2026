...
int main(int argc, char **argv) {
    ...
    Float crease_angle = -1, scale = -1, adaptivity = 0.0f;
    std::string batchOutput, contour_file;
    ...
    try {
        for (int i=1; i<argc; ++i) {
            if (strcmp("--fullscreen", argv[i]) == 0 || strcmp("-F", argv[i]) == 0) {
                ...
                batchOutput = argv[i];
            } else if (strcmp("--adaptivity", argv[i]) == 0 || strcmp("-a", argv[i]) == 0) {
                if (++i >= argc) {
                    cerr << "Missing adaptivity argument!" << endl;
                    return -1;
                }
                adaptivity = str_to_float(argv[i]);
            } else if (strcmp("--contour", argv[i]) == 0) {
                if (++i >= argc) {
                    cerr << "Missing contour file argument!" << endl;
                    return -1;
                }
                contour_file = argv[i];
    ...
    if (args.size() > 1 || help || (!batchOutput.empty() && args.size() == 0)) {
        ...
        cout << "   -a, --adaptivity <factor> Curvature adaptivity [0.0 = uniform, 1.0 = highly adaptive]" << endl;
        cout << "   --contour <file.obj>      Guide contour lines/loops to constrain topology flow" << endl;
